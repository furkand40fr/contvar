...
\textbf{Structure sources.} For the selected DMS wild-type and variant sequences, three-dimensional structures were generated with AlphaFold3. To reduce folding cost on the compute cluster, available assay/MSA resources were reused instead of regenerating alignments from scratch. For the larger GO protein set used in phase~0 and decoder embedding export, residue graphs were built from already available structures, primarily AlphaFold Database models when present and alternative structure resources otherwise. All structures are treated as fixed mmCIF/CIF inputs for graph construction.

\textbf{From structures to residue graphs.} Per-residue $\mathrm{C}_\alpha$ coordinates and amino-acid identities are extracted from monomer structures and assembled into PyTorch Geometric graphs. Connectivity uses the hybrid edge scheme below; an optional pure $k$NN baseline (Graphein-style) is available for ablations.
