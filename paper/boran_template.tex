...
\begin{enumerate}
    \item \textbf{Mutation-aware graph encoder.} Training proceeds in two phases. \textbf{Phase~0} performs GO semantic-similarity triplet pretraining with ontology-specific heads (MF, BP, CC) on prebuilt residue graphs. \textbf{Phase~1} specializes the same backbone on DMS protein-family triplets with a joint global graph-level and local mutation-site contrastive loss; streaming semi-hard negative mining is active from the first phase~1 epoch (there is no separate warm-up pass over the full DMS corpus before mining).
...
\textbf{DMS (phase~1) track.} The mutagenesis corpus is organized by protein family with one wild-type anchor, multiple positives, and multiple negatives per family. The fixed split file shipped with the project contains 97 families (93 train, 2 validation, 2 test) to avoid family leakage across splits. Raw structures are consumed as mmCIF/CIF files, and per-residue ESM-2 embeddings are loaded from a precomputed store keyed by normalized protein identifiers. Mutated residue positions are parsed from variant filenames during collation and mining. ProteinGym-style curation~\cite{notin2023proteingym} informs this triplet organization.

\textbf{GO (phase~0) track.} Phase~0 consumes prebuilt residue graphs and ontology-specific semantic-similarity tables. Positives use similarity $\geq 0.8$; negatives use similarity $\leq 0.2$ and are sampled from low/mid bins ([0.0,0.1), [0.1,0.2]) for hardness balancing. The loader also drops triplets whose proteins are missing in the prebuilt graph index.
...
After phase~0, phase~1 loads DMS protein-family triplets under a fixed train/validation/test split (97 families: 93 train, 2 validation, 2 test), initializes weights from the phase~0 checkpoint, and runs one contrastive training loop for 200 epochs by default. Training does \textbf{not} alternate between a full-corpus epoch without mining and a later mining phase: every step performs streaming semi-hard negative selection and rebuilds batches each epoch.
...
Training proteins are partitioned by UniRef50 cluster so that no cluster appears in more than one of train, validation, or test; the same partition is shared with phase~0 evaluation where applicable. Each protein is represented by a multi-hot label vector over the aspect vocabulary.
